...
import { render as renderAll, getPinSizeForZoom } from './modules/render.js';
...
// ========== Map ==========
// maxZoom 21 für Detail-Ansicht (gpx.studio-Niveau). Tiles werden über maxNativeZoom
// hinaus hochskaliert — das kostet etwas Schärfe, erlaubt aber präziseres Pin-Setzen.
const map = L.map('map', { zoomControl: false, tap: true, maxZoom: 21 }).setView([52.4512, 13.2890], 18);
L.control.zoom({ position: 'bottomright' }).addTo(map);

// Straßenkarte: CARTO Voyager — detailreicher als OSM.de, zeigt mehr Wege/Parzellen,
// saubere Typografie. Native bis Zoom 20, Interpolation bis 21.
...
// Satellit: Esri World Imagery bleibt, aber tiefer überzoombar.
...
const ctx = {
  ...
  updateTotal,
  pushUndo: () => {} // wird gleich mit undoManager überschrieben
};

// Undo-Manager & Shortcuts
const undoManager = createUndoManager(ctx);
ctx.pushUndo = () => undoManager.push();
ctx.undo = () => undoManager.undo();
ctx.redo = () => undoManager.redo();

window.addEventListener('keydown', (e) => {
  // Nicht abfangen wenn in Input/Textarea getippt wird
  ...
  if (e.key === 'z' || e.key === 'Z') {
    e.preventDefault();
    if (e.shiftKey) {
      if (undoManager.redo()) showInfo('↷ Wiederhergestellt');
      else showInfo('Nichts zum Wiederherstellen', 'err');
    } else {
      if (undoManager.undo()) showInfo('↶ Rückgängig');
      else showInfo('Nichts zum Rückgängig-Machen', 'err');
    }
...
// Undo/Redo Buttons in der Searchbar
...
// Erzeugt einen formatierten Timestamp wie 2026-04-22_2145 für Dateinamen.
...
if (snapBtn) {
  snapBtn.onclick = async () => {
    // Zoom-Guard: unter Zoom 17 zeigen Pins nur kleine Dots — warnen vor "leerem" PDF/PNG.
    if (map.getZoom() < 17 && state.objects.length > 0) {
      if (!confirm(`Zoomstufe ist ${map.getZoom()} — Pins werden nur als kleine Punkte dargestellt. Näher reinzoomen und erneut Snapshot erstellen?\n\nOK = trotzdem speichern, Abbrechen = zurück.`)) return;
    }
    ...
    try {
      ...
      canvas.toBlob(b => {
        ...
        a.href = url;
        a.download = `UnionE_Snapshot_${state.meta.we || 'WE'}_${formatStamp()}.png`;
        a.click();
...
// Zoom-adaptive Pins: bei Zoom-Änderung über die Stufengrenzen hinweg neu rendern
// Schwellen: 17 (sm→md) und 19 (md→lg). Nur neu zeichnen wenn Stufe wechselt,
// damit wir nicht bei jedem kleinen Zoom-Tick die ganze Map neu malen.
let lastPinSize = getPinSizeForZoom(map.getZoom());
map.on('zoomend', () => {
  const s = getPinSizeForZoom(map.getZoom());
  if (s !== lastPinSize) {
    lastPinSize = s;
    renderAll(ctx);
  }
...
// Registriert den Service Worker für Offline-Nutzung. Update-Erkennung:
// bei neuer SW-Version wird der User gefragt, ob er reloaden möchte.
      ...
      reg.addEventListener('updatefound', () => {
        ...
        nw.addEventListener('statechange', () => {
          if (nw.state === 'installed' && navigator.serviceWorker.controller) {
            // Neue Version verfügbar — Hinweis zeigen
            showInfo('Neue Version verfügbar · Seite neu laden');
          }
        });
      });
    }).catch(err => {
...
// Registriert sich nur auf https oder localhost (Sicherheits-Vorgabe der Browser).
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
