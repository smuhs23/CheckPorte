...
import { showInfo, fmt, fmtEur } from './ui.js';
  ...
  if (withTracePages && state.traces.length > 0) {
    state.traces.forEach((tr, ti) => {
      ...
      if (cableRows.length > 0) {
        cableRows.push(['', '', '', '', '', '', 'Σ Kabel', fmt(row.tC)]);
      } else {
        cableRows.push(['', 'Keine Leitungen erfasst', '', '', '', '', '', '0,00']);
      }
      doc.autoTable({
        startY: tableY,
        head: [['LV', 'Leitungstyp', 'Anzahl', 'Reserve', 'Trassenlänge', 'Eff. Länge', '€/m', 'Summe €']],
        body: cableRows,
        theme: 'grid',
        headStyles: { fillColor: NAVY, textColor: 255, fontStyle: 'bold', fontSize: 8 },
        bodyStyles: { fontSize: 8, cellPadding: 2 },
